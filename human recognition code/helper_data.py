...
def func(im_path, an_path):
    ...
    for txt in filenames:
        ...
        for row in range(x.shape[0]):
            height.append(x[row, 2])
            width.append(x[row, 3])
...
# sliki, anotacii = func(r"C:\Users\Koki\Desktop\LSTM\frames\train",r"C:\Users\Koki\Desktop\LSTM\annotations\train")
# for img in sliki:
#     for anx in anotacii.pop(0):
#         cv2.rectangle(img, (anx[1], anx[0]), (anx[3],anx[2]), color=(255, 0, 0))
#         cv2.imshow("img", img)
#         print(anx[0],anx[1],anx[2],anx[3])
#         cv2.waitKey(0)
# # X,Y H,W
...
def get_anchor_data_ssd(bboxes, anchor_dims, img_dims, anchor_stride, iou_low, iou_high, num_negs_ratio):
    ...
    for img_ind, img_bboxes in tqdm(enumerate(bboxes)):
        ...
        output_dims_class = (np.int(img_dims[0] / anchor_stride), np.int(img_dims[1] / anchor_stride),
                             num_anchors + 1)  # each depth-wise matrix contains classes for one anchor size, last matrix contains negative samples
        output_dims_reg = (np.int(img_dims[0] / anchor_stride), np.int(img_dims[1] / anchor_stride),
                           num_anchors * 4)  # regressor output - 4 values: delta_r, delta_c, delta_h, delta_w

        # output matrices for one image
        output_class = np.zeros(output_dims_class).astype(np.int)
        output_reg = np.zeros(output_dims_reg).astype(np.float)
        ...
        for output_row, center_row in enumerate(
                range(start_r, img_dims[0] - start_r - 1, anchor_stride)):  # iterate through rows of centers
            for output_col, center_col in enumerate(
                    range(start_c, img_dims[1] - start_c - 1, anchor_stride)):  # iterate through columns of centers
...
def save_results(results_path, images, output_cls, output_reg, anchor_dims, anchor_stride, prob_thr, reg_norm_coef):
    ...
    for im_ind, image in enumerate(images):
        ...
        for pred_ind in range(len(r)):  # iterate over positive predictions
            ...
            delta_r = delta_c = delta_h = delta_w = 0
            ...
            if (max_row_adj - min_row_adj) > 0 and (max_col_adj - min_col_adj) > 0:
                ...
                to_add.append([min_col_adj, min_row_adj, max_col_adj, max_row_adj])
        boxes.append(to_add)    
...
def save_results_final(results_path, images, coordinates):
    finalResPath= r'C:\Users\petar\Desktop\diogen\finalres'
    for im_ind, image in enumerate(images):
        coordinates=np.array(coordinates)
        r = coordinates
        for pred in range(len(r[im_ind])):  # iterate over positive predictions
                # plot bounding box onto image
               # if (r[im_ind][pred][3] - r[im_ind][pred][1]) > 0 and (r[im_ind][pred][2]- r[im_ind][pred][0]) > 0:
                    # cv2.circle(image, (center_col, center_row), 3, color=(255, 0, 0), thickness=3)     # plot object centers
                    cv2.rectangle(image, (r[im_ind][pred][0], r[im_ind][pred][1]), (r[im_ind][pred][2],r[im_ind][pred][3]), color=(0, 0, 255),thickness=1)         
        cv2.imwrite(os.path.join(finalResPath, str(im_ind).zfill(4) + '.bmp'), image)
